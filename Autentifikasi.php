...
public function gagal()
{
    ...
    public function index()
    {
        ...
        if($this->form_validation->run() == false){
            $data['judul'] = 'login';
            $data['user']='';
        //kata login merupakan nilai dari variabel judul dalam array d$ata yang dikirimkan ke view aute_header
...
$this->form_validation->set_rules('email',  'Alamat  Email', 'required|trim|valid_email|is_unique[user.email]', [
    ...
    private function _login ()
    {
        ...
        if ($user){
            ...
            if($user['is_active'] == 1){
                //cek password
                if (password_verivy($password, $user['password'])){
                    $data = [
                        'email'=> $user['email'],
                    ...
                    $thiss->session->set_flashdata('pesan','<div class="alert alert-danger alert-message" role="alert">user belum diaktifasi</div>');
                    redirect ('autentifikasi');
                }
            }else{
